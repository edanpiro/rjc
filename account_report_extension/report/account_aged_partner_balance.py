...
class aged_trial_report_ext(aged_trial_report):
    ...
    def _get_lines(self, form):
        ...
        if self.direction_selection == 'future':
            # testing: Change in "AND (COALESCE(l.date_maturity, l.date) <= %s)", change from < to <=
            self.cr.execute('SELECT l.partner_id, SUM(l.debit-l.credit) \
                        FROM account_move_line AS l, account_account, account_move am \
                        WHERE (l.account_id=account_account.id) AND (l.move_id=am.id) \
                        AND (am.state IN %s)\
        ...
        for i in range(5):
            ...
            start = form[str(i)]['start']
            stop = form[str(i)]['stop']
            if form[str(i)]['start'] and form[str(i)]['stop']:
                dates_query += ' BETWEEN %s AND %s)'
                args_list += (form[str(i)]['start'], form[str(i)]['stop'])
            elif form[str(i)]['start']:
                dates_query += ' >= %s)'  # testing: >= not >
...
class report_agedpartnerbalance(osv.AbstractModel):
    ...
    _wrapped_report_class = aged_trial_report_ext
